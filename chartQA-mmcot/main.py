...
def parse_args():
    ...
    parser.add_argument('--prompt_format', type=str, default='QCM-A', help='prompt format template',
                        choices=['QCM-A', 'QCM-E', 'QCM-LE', 'QCMG-A', 'QCM-LEA', 'QCM-ALE','QA'])
...
def T5Trainer(
    ...
    qids = dataframe['qids']
    # Handle ScienceQA-style splits vs custom datasets
    if isinstance(qids, dict):
    # ScienceQA: predefined splits
        train_qids = qids.get('train', [])
        val_qids = qids.get('val', [])
        test_qids = qids.get('test', [])
    else:
    # Custom dataset: use all samples for training
        train_qids = qids
        val_qids = []
        test_qids = []

    print(f"🧠 Using {len(train_qids)} training samples, {len(val_qids)} validation, {len(test_qids)} test.")
